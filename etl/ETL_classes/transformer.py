...
class Transformer:
    ...
    def transform(self, extracted_part: dict) -> list[dict]:
        ...
        Returns:
            list[dict]: возвращаем преобразованные данные
            в виде списка словарей для ElasticSearch
        """
        ...
        for row in extracted_part:
            filmwork = {
                ...
                "actors_names": row['actors_names']
                        if row['actors_names'] is not None else '',
                "writers_names": row['writers_names']
                        if row['writers_names'] is not None else '',
